...
export class HeroesService {
  ...
  getHeroeById(id: number) {
    return this.heroes.find((heroe) => heroe.id == id); //id es un string
  }

  buscarHeroes(termino: string): Heroe[] {
    termino = termino.toLowerCase();
    const heroeArr: Heroe[] = this.heroes.filter((heroe) =>
      heroe.nombre.toLocaleLowerCase().includes(termino)
    );
    return heroeArr;
  }
}
...
export interface Heroe {
  //para que tengan esta misma interfaz
  id: number;
  nombre: string;
  bio: string;
  img: string;
